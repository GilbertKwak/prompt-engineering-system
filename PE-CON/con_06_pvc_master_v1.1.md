...
<ve_engine_prompt version="2.1">
  ...
  <note>투자 의사결정 필요 시 FIN-06 VA-MASTER 호출 (fin06_trigger 조건 참조)</note>
  <auto_input>대상 미입력 → 맥락 자동 추론 + [추론 근거] 명시</auto_input>
  <workflow>
...
## 🔷 PVC-MASTER · 오케스트레이터 v1.1

```xml
<pvc_master_prompt version="1.1">
  ...
  <integrated_workflow>
    ...
    PHASE 4 [투자판단] → fin06_trigger 조건 평가 후 FIN-06 VA-MASTER 연동
    PHASE 5 [실행계획] → Action Plan: 주차별·담당자·KPI
  </integrated_workflow>

  <!-- v1.1 신규: FIN-06 호출 트리거 조건 명시 -->
  <fin06_trigger>
    <description>PHASE 4에서 아래 조건 중 1개 이상 해당 시 FIN-06 VA-MASTER 자동 호출</description>
    <condition id="1">Value Index &lt; 1.0 (기능 대비 비용 과중)</condition>
    <condition id="2">예상 ROI &lt; 15% (투자 타당성 임계값 미달)</condition>
    <condition id="3">대안 비용 영향 &gt; ±20% (중대 원가 구조 변동)</condition>
    <condition id="4">의사결정 분류 = ELIMINATE (자산·기능 폐기 결정 포함)</condition>
    <condition id="5">투자 규모 &gt; 5억 원 (또는 맥락상 대형 CAPEX 판단 시)</condition>
    <fallback>위 조건 미해당 시 PVC-03 의사결정 매트릭스로 자체 완결</fallback>
  </fin06_trigger>
  ...
  <command_shortcuts>
    ...
    PVC-DEAL [딜명] [섹터]     → 반도체·AI 딜 특화 FULL 실행 (fin06_trigger 자동 활성)
  ...
  <quality_gates>
    ...
    - KPI 측정 불가 목표 금지
    - 추상 표현 금지
    - fin06_trigger 조건 미평가 상태에서 투자 판단 금지
    - FIN-06 호출 시 트리거 조건 번호 명시 필수
  </quality_gates>
</pvc_master_prompt>
```
...
# PE-CON 딜 특화 (v1.1 신규)
"PVC-DEAL [딜명] [섹터]"       → 반도체·AI 딜 특화 PVC FULL + fin06_trigger 자동 활성
...
## 📊 기존 7종 vs PVC-MASTER v1.1 적용 전후

| 항목 | 기존 7종 | PVC-MASTER v1.1 |
|---|---|---|
| 파일 수 | 7개 (+중복 1) | 4개 |
...
| 명령어 | 없음 | 7종 단축 명령어 |
| FIN-06 트리거 조건 | 없음 | 5개 조건 명시 (v1.1 신규) |
| PVC-DEAL 명령어 | 없음 | XML + 사용 섹션 완전 등록 (v1.1 신규) |

**적용 결정: ✅ PVC-MASTER v1.1 즉시 채택.**
...
- **연동 1**: ⚙️ FIN-06 VA-MASTER v1.0 — PHASE 4 fin06_trigger 조건 충족 시 호출
...
| 버전 | 날짜 | 변경 내용 |
|---|---|---|
| v1.0 | 2026-05-16 | 최초 등록. 기존 7종 통합. PVC-01/02/03 + MASTER 4종 재편 |
| v1.1 | 2026-05-17 | 오타 3건 수정(낭비·낮음·과잉). PVC-DEAL 명령어 XML 등록. fin06_trigger 조건 블록 신규 추가. quality_gates FIN-06 조건 강화 |

---

*등록: 2026-05-16 KST | 업데이트: 2026-05-17 KST | Gilbert | PE-CON STRATEGIC THINKER OS*
