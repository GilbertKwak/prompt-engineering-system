...
tags: [FIN, investment, VC, CSO, board, JV, ESG, SEMI, GAS, OIL, orchestrator]
derivatives:
  - FIN-MSIA-SEMI
  - FIN-MSIA-JV
  - FIN-MSIA-ESG
...
> **PE-3 개선**: 68.5% (미승인) → **예상 95%+** (승인)  
> **v2.1 변경**: 파생 5종 체계 완성 (GAS·OIL 추가), PE-3 점수 목표 95%+로 상향  
> **SSOT**: [Notion PE-FIN 라이브러리](https://notion.so/34f55ed436f081c2ad05df1dc11e0ae7)

---

## PE-3 자동검증 결과 — v2.0 vs v2.1

| 검증 항목 | v2.0 | v2.1 최적화 |
|---|---|---|
| ROLE 명확성 | ✅ 단일 Orchestrator | ✅ 유지 |
| 입력 파라미터화 | ✅ 5변수 구조화 | ✅ DOMAIN 확장 (GAS·OIL 추가) |
| 단계적 분석 체인 | ✅ 7-Step | ✅ 7-Step 유지 |
| 출력 포맷 정의 | ✅ Notion MD + GitHub Issue | ✅ 유지 |
| PE-1 수치 출처 | ✅ 강제 규칙 내장 | ✅ 유지 |
| PE-3 Bear 시나리오 | ✅ Step7 강제 포함 | ✅ 에너지 도메인 Bear 확장 |
| E-0N 오류 감지 | ✅ E-02/04/07 내장 | ✅ E-09(에너지가격 급변) 추가 |
| SSOT 연결 | ✅ Notion + GitHub | ✅ 파생 5종 경로 명시 |
| 파생 프롬프트 체계 | 3종 | **5종 완성 (GAS·OIL 추가)** |
| **종합 점수** | **예상 93%+** | **예상 95%+** |
...
| 파생 ID | 명칭 | 특화 도메인 | 연계 스텝 | 버전 |
|---|---|---|---|---|
| `FIN-MSIA-SEMI` | 반도체 투자 심사 특화 | HBM·OSAT·EUV·NOR Flash | Step3+Step5 | v1.1 |
| `FIN-MSIA-JV` | JV/합작투자 전략 심사 | B-Star·AstraChips·Global JV | Step4+Step6 | v1.1 |
| `FIN-MSIA-ESG` | ESG·대안투자 심사 | sCO₂·청정에너지·AI 인프라 | Step2+Step7 | v1.1 |
| `FIN-MSIA-GAS` | 가스·LNG 에너지 투자 심사 | LNG·PNG·가스전·트레이딩 | Step3+Step5+Step7 | v1.1 |
| `FIN-MSIA-OIL` | 석유·정유 에너지 투자 심사 | 원유·정유·업스트림·다운스트림 | Step3+Step5+Step7 | v1.1 |
...
```xml
<MasterStrategicInvestmentAnalysisAgent version="2.1" id="FIN-MSIA-MASTER">
  ...
  <SystemRole>
    ...
    도메인 특화: HBM·sCO₂·AstraChips·JV·LNG·원유·정유 (Gilbert Kwak Domain)
  </SystemRole>
  ...
  <InputParameters>
    ...
    {{DOMAIN}}       <!-- SEMI | FIN | JV | ESG | GAS | OIL | CON | EQP -->
  ...
  <ErrorPrecheck>
    ...
    E-09: 에너지 가격 급변 감지 → GAS/OIL 도메인 시 유가·가스가격 데이터 필수 확인
  ...
  <AutoTrigger>
    ...
    DOMAIN=GAS 시 FIN-MSIA-GAS 자동 라우팅.
    DOMAIN=OIL 시 FIN-MSIA-OIL 자동 라우팅.
  </AutoTrigger>
  ...
  <Workflow>
    ...
    <Step3_ProjectEvaluation>
      ...
      에너지 도메인(GAS/OIL): 매장량·회수율·OPEX/CAPEX·유가 민감도 추가
    ...
    <Step7_FinalStrategicRecommendation>
      ...
      <BearScenario>
        ...
        에너지 도메인 추가: 유가·가스가격 급락 시나리오 (FIN-07/08 연동)
      </BearScenario>
    </Step7_FinalStrategicRecommendation>
  ...
  <!-- DOMAIN ROUTER -->
  <DomainRouter>
    DOMAIN=SEMI → FIN-MSIA-SEMI 자동 라우팅
    DOMAIN=JV   → FIN-MSIA-JV 자동 라우팅
    DOMAIN=ESG  → FIN-MSIA-ESG 자동 라우팅
    DOMAIN=GAS  → FIN-MSIA-GAS 자동 라우팅 (신규 v2.1)
    DOMAIN=OIL  → FIN-MSIA-OIL 자동 라우팅 (신규 v2.1)
    DOMAIN=범용 → MASTER 직접 실행
  </DomainRouter>
  ...
  <CrossReference>
    ...
    FIN-07: 가스 에너지 분석 → FIN-MSIA-GAS 연동 (신규)
    FIN-08: 석유 정유 분석 → FIN-MSIA-OIL 연동 (신규)
  ...
  <SSOTConnection>
    ...
    GitHub: prompts/applied-cases/investment-decision/fin_msia_master_v2.1.md
  ...
  <QualityControl>
    ...
    ✓ Bear 시나리오 포함 여부
    ✓ PE-1 수치 출처 명시 여부
    ✓ 에너지 도메인 시 유가·가스가격 데이터 포함 여부 (v2.1 추가)
  </QualityControl>
...
```bash
# FIN-MSIA-MASTER v2.1 실행 (GAS 도메인 라우팅)
python automation/run_prompt.py \
  --prompt prompts/applied-cases/investment-decision/fin_msia_master_v2.1.md \
  --domain "GAS" \
  --stage "Pre-IPO" \
  --depth "Deep" \
  --lang "KR+EN"
...
python automation/auto_validate.py \
  --file prompts/applied-cases/investment-decision/fin_msia_master_v2.1.md \
  --rules PE-1,PE-3
```
...
| 버전 | 날짜 | 주요 변경 사항 | 작성자 |
|---|---|---|---|
| v2.1 | 2026-05-08 | 파생 5종 체계 완성 (GAS·OIL 신규 추가), E-09 에너지 오류 감지 추가, DomainRouter 신설, PE-3 목표 95%+로 상향 | Gilbert Kwak |
| v2.0 | 2026-05-08 | PE-3 최적화 전면 개정 (68.5%→93%+), Bear 시나리오 강제, E-0N 내장, SSOT 연결 | Gilbert Kwak |
